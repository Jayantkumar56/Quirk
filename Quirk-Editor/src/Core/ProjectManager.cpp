...
namespace Quirk {
    ...
    std::filesystem::path ProjectManager::AddRecentProject(const std::filesystem::path& directory) {
        // find the proj file :-
    ...
    Ref<Project> ProjectManager::CreateNewProject(ProjectMetadata& projMetaData) {
        ...
        // creating the project folder structure and required files
        {
        ...
        auto project = Project::Create<EditorAssetManager>(projFilePath, std::move(newConfig));
